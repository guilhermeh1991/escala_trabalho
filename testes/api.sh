...
<?php return [
  'bd_host'=>'localhost', 'bd_nome'=>'teste_escala',
...
HASH=$(mysql teste_escala -sN -e "SELECT senha_hash FROM usuarios WHERE email='ana@teste.com';")
case "$HASH" in
...
TOKEN=$(mysql teste_escala -sN -e "SELECT token_confirmacao FROM usuarios WHERE email='ana@teste.com';")
...
PAPEL=$(mysql teste_escala -sN -e "SELECT papel FROM usuarios WHERE email='ana@teste.com';")
...
TOKEN_B=$(mysql teste_escala -sN -e "SELECT token_confirmacao FROM usuarios WHERE email='bruno@outra.com';")
...
N=$(mysql teste_escala -sN -e "SELECT COUNT(*) FROM colaboradores WHERE loja_id='$LOJA_A';")
[ "$N" = "2" ] && ok "2 colaboradores gravados" || falha "gravou $N colaboradores"
NA=$(mysql teste_escala -sN -e "SELECT COUNT(*) FROM ausencias;")
...
NOME=$(mysql teste_escala -sN -e "SELECT nome FROM lojas WHERE id='$LOJA_A';")
...
mysql teste_escala -e "SELECT 1;" > /dev/null 2>&1 && ok "banco intacto após tentativa" || falha "banco afetado"

echo ""
echo "=== 14. Auditoria ==="
NR=$(mysql teste_escala -sN -e "SELECT COUNT(*) FROM registro_acoes;")
